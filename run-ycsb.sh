...
pmem_dir="/home/deadpool/HUNTER-REPO/MadFS/tmp"
...
load_workload() {
    ...
    if [[ "${file_system}" =~ "SplitFS" ]]; then
        ...
        LD_PRELOAD=./build-debug/libmadfs.so $leveldb_build_dir/db_bench --use_existing_db=0 --benchmarks=ycsb,stats,printdb --db=$database_dir --threads="$threads" --open_files=1000 2>&1 | tee $output/$fs/$threads/$tracefile
    else
...
run_workload() {
    ...
    if [[ "${file_system}" =~ "SplitFS" ]]; then
        ...
        export LD_LIBRARY_PATH=/usr/local/lib64:$LD_LIBRARY_PATH
        LD_PRELOAD=./build-debug/libmadfs.so $leveldb_build_dir/db_bench --use_existing_db=1 --benchmarks=ycsb,stats,printdb --db=$database_dir --threads="$threads" --open_files=1000 2>&1 | tee $output/$fs/$threads/$tracefile
    else
